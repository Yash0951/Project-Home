...
function TreeSVG({ totalDays, streak, newLeafIndex, comfortMode }: TreeSVGProps) {
  // Tree leaves grow with total days studied to maintain overall tree progress permanently
  const leaves = Math.min(totalDays, LEAF_POSITIONS.length);
  const flowers = totalDays >= 7 ? Math.min(Math.floor((totalDays - 7) / 5) + 1, FLOWER_POSITIONS.length) : 0;
  const birds = streak >= 7 ? Math.min(Math.floor(streak / 7), 5) : 0;
  ...
  return (
    <svg viewBox="0 0 200 280" className="w-full max-w-xs mx-auto">
      ...
      {/* Branches */}
      {totalDays >= 3 && <path d="M88,145 Q68,128 48,135" fill="none" stroke={trunkColor} strokeWidth="2.5" strokeLinecap="round" />}
      {totalDays >= 6 && <path d="M112,145 Q132,125 152,132" fill="none" stroke={trunkColor} strokeWidth="2.5" strokeLinecap="round" />}
      {totalDays >= 9 && <path d="M94,120 Q64,100 44,108" fill="none" stroke={trunkColor} strokeWidth="2" strokeLinecap="round" />}
      {totalDays >= 12 && <path d="M106,120 Q136,95 156,102" fill="none" stroke={trunkColor} strokeWidth="2" strokeLinecap="round" />}
      {totalDays >= 15 && <path d="M100,108 Q78,85 63,90" fill="none" stroke={trunkColor} strokeWidth="1.5" strokeLinecap="round" />}
      {totalDays >= 18 && <path d="M100,108 Q122,82 137,88" fill="none" stroke={trunkColor} strokeWidth="1.5" strokeLinecap="round" />}

      {/* Leaves */}
      {LEAF_POSITIONS.slice(0, leaves).map((pos, i) => {
        const isNew = i === newLeafIndex;
        const r = Math.min(4 + (i < 10 ? 2 : i < 20 ? 1 : 0), 6);
        ...
        <motion.g key={`flower-${i}`}>
          {[0, 60, 120, 180, 240, 300].map((angle, j) => (
            <motion.ellipse
              ...
              initial={{ scale: 0, opacity: 0 }}
              animate={{ scale: 1, opacity: 0.9 }}
              transition={{ duration: 0.5, delay: i * 0.2 + j * 0.05 }}
            />
          ))}
          <motion.circle
            cx={pos.x}
      ...
      {birds > 0 && [...Array(birds)].map((_, i) => (
        ...
        >
          <path
            d={`M${15 + i * 40},${15 + i * 10} Q${11 + i * 40},${12 + i * 10} ${8 + i * 40},${15 + i * 10} M${15 + i * 40},${15 + i * 10} Q${19 + i * 40},${12 + i * 10} ${22 + i * 40},${15 + i * 10}`}
            fill="none"
            stroke={comfortMode ? '#C9B8E8' : '#8B6F47'}
            strokeWidth="1.2"
            strokeLinecap="round"
          />
        </motion.g>
      ...
      {/* Fireflies */}
      {streak >= 14 && [...Array(Math.min(streak - 13, 8))].map((_, i) => (
        <motion.circle
          key={`firefly-${i}`}
          cx={20 + (i * 23) % 160}
          cy={30 + (i * 17) % 80}
...
export default function ProgressTree() {
  ...
  const handleMarkStudied = () => {
    ...
    setNewLeafIndex(Math.min(nextTotalDays - 1, LEAF_POSITIONS.length - 1));
    markStudiedToday();
    setJustMarked(true);
    setTimeout(() => setJustMarked(false), 3000);
    setTimeout(() => setNewLeafIndex(-1), 2000);
  ...
  return (
    <section id="progress" className="relative min-h-screen py-20 px-4">
      <motion.div
        ...
        className="text-center mb-12"
      >
        <h2 className={`font-[Great_Vibes] text-4xl md:text-5xl mb-2 ${comfortMode ? 'text-moonlight glow-text-light' : 'text-rose-gold glow-text'}`}>
          Progress Tree 🌱
        </h2>
        <p className={`font-[Poppins] text-sm ${comfortMode ? 'text-lavender/60' : 'text-warm-brown/50'}`}>
          every day you study, this tree grows
      ...
      <div className="max-w-md mx-auto">
        ...
        >
          ...
          <div className="text-center mt-4">
            <p className={`font-[Dancing_Script] text-xl ${comfortMode ? 'text-starlight' : 'text-rose-gold'}`}>
              {totalDays === 0
                ? "Your tree is waiting for its first leaf 🌱" 
                : totalDays < LEAF_POSITIONS.length 
                  ? `${totalDays} ${totalDays === 1 ? 'leaf' : 'leaves'} — keep growing! 🌿`
                  : "Your tree is in full bloom! 🌳✨"}
            </p>
            <div className="flex flex-wrap items-center justify-center gap-2 mt-3">
              {totalDays >= 3 && <span className="text-xs">🌿 Branches</span>}
              {totalDays >= 7 && <span className="text-xs">🌸 Flowers</span>}
              {studyStreak >= 7 && <span className="text-xs">🐦 Birds</span>}
              {studyStreak >= 14 && <span className="text-xs">✨ Fireflies</span>}
            </div>
          </div>
        </motion.div>
