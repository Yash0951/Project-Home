...
export default function EmergencyButton() {
  const { showEmergency, setShowEmergency } = useApp();
  const [visibleMessages, setVisibleMessages] = useState(0);

  // Lock background scroll when emergency overlay is open
  useScrollLock(showEmergency);
  ...
  return (
    <>
      {/* 1. Main Page Pill Button (Matches your original design) */}
      <div className="flex flex-col items-center justify-center my-8">
        <p className="text-xs text-lavender/40 font-[Poppins] mb-3">✿ ~ ✿</p>
        
        <motion.button
          onClick={() => setShowEmergency(true)}
          className="px-8 py-3.5 rounded-full bg-red-500/10 border border-red-500/30 text-red-300 font-[Dancing_Script] text-xl flex items-center gap-3 shadow-lg hover:bg-red-500/20 transition-all duration-300"
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          style={{ 
        ...
        >
          ...
          <span>Press only when everything feels impossible</span>
        </motion.button>

        <p className="text-xs text-lavender/40 font-[Poppins] mt-3">✿ ~ ✿</p>
      </div>

      {/* 2. Full-Screen Emergency Modal Overlay */}
      <AnimatePresence>
        {showEmergency && (
          <motion.div
            initial={{ opacity: 0 }}
              ...
              {visibleMessages >= emergencyMessages.length && (
                ...
                >
                  ...
                  >
                    <a
                      href="tel:+918821927909"
                      className="relative block w-20 h-20 rounded-full flex items-center justify-center bg-red-500/20 border-2 border-red-400/40 hover:bg-red-500/30 hover:border-red-400/60 transition-colors duration-300 no-underline"
                      style={{
                        textDecoration: 'none',
                        boxShadow: '0 0 30px rgba(239,68,68,0.2), 0 0 60px rgba(239,68,68,0.1)',
                      }}
                  ...
                  <div className="text-center">
                    <p className="font-[Dancing_Script] text-lg text-red-300 glow-text-light">
                      Call me — I'm here for you
                    </p>
                  </div>
                  ...
                  <motion.button
                    onClick={handleClose}
                    className="mt-4 px-6 py-3 rounded-full glass-dark text-moonlight font-[Dancing_Script] text-lg border border-lavender/20"
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                  >
                    I'll stand up again tomorrow 💪
